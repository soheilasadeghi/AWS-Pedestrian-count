#!/bin/bash
if [[ $# -eq 0 ]] ; then
    echo 'Please enter: ./setup_infra.sh soheila-s3-bucket'
    exit 0
fi
...
echo '{
    ...
    "Statement": [
        ...
        },
        {
            ...
            "Resource": "arn:aws:s3:::'soheila-s3-bucket'/*"
        }
    ]
}' > ./policy
...
echo "Creating bucket "soheila-s3-bucket""
aws s3api create-bucket --acl public-read-write --bucket soheila-s3-bucket --output text >> setup.log
...
echo "Sleeping 15 seconds to allow policy to attach to role"
sleep 15
...
echo "Creating cloudwatch rule to schedule lambda every 60 minutes"
aws events put-rule --name my-scheduled-rule --schedule-expression 'rate(60 minutes)' --output text >> setup.log
