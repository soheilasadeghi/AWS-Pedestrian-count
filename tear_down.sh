...
if [[ $# -eq 0 ]] ; then
    echo 'Please enter  ./setup_infra.sh to remove related settings for this project'
    exit 0
fi
...
echo "Deleting bucket soheila-s3-bucket"
aws s3 rm s3://soheila-s3-bucket --recursive --output text >> tear_down.log
aws s3api delete-bucket --bucket soheila-s3-bucket --output text >> tear_down.log
